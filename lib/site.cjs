...
const resourceCategories = [
  ...
  },
  {
    ...
    cards: [
      ...
      {
        name: 'E-file.com',
        description: 'Low-cost online tax filing for federal and state returns. Filing missing returns is often the required first step before the IRS will approve a payment plan or release a levy.',
        bestFor: 'Best for people who need to catch up on unfiled tax returns or file the current year while dealing with IRS levy risk.',
        href: '/go/e-file',
        affiliate: true
      }
    ]
  },
  {
    ...
    cards: [
      ...
      {
        name: 'LawDepot',
        description: 'Do-it-yourself legal document templates — garnishment answers, hardship letters, payment agreements, and demand letters you can complete without hiring an attorney.',
        bestFor: 'Best for people who need a properly formatted legal document fast and cannot justify attorney fees for routine paperwork.',
        href: '/go/lawdepot',
        affiliate: true
      }
    ]
  },
  {
    ...
    cards: [
      ...
      {
        name: 'SmartCredit',
        description: 'Credit monitoring and score tools that let you track all three bureaus, spot judgment-related damage, and watch your score recover as you resolve the underlying debt.',
        bestFor: 'Best for people who want to actively monitor their credit while working through a garnishment or judgment.',
        href: '/go/smartcredit',
        affiliate: true
      },
      {
        name: 'WalletHub',
        description: 'Free credit scores, reports, and monitoring with daily updates — a no-cost way to keep an eye on where you stand while you rebuild.',
        bestFor: 'Best for people who want free ongoing credit visibility without committing to a paid monitoring service.',
        href: '/go/wallethub',
        affiliate: true
      }
    ]
...
const affiliateRedirects = {
  ...
  // CJ links (GarnishmentCalculator.com property 101734067) — added July 8, 2026
  'e-file': 'https://www.jdoqocy.com/click-101734067-11917161',
  'smartcredit': 'https://www.kqzyfj.com/click-101734067-16982685',
  'wallethub': 'https://www.anrdoezrs.net/click-101734067-16971004',
  'lawdepot': 'https://www.anrdoezrs.net/click-101734067-16945498',
...
// The weekly earnings floor fully protected from consumer-debt garnishment.
// Mirrors lib/garnish-math.cjs so this table can never drift from the calculator.
// Returns null for states that prohibit consumer-debt wage garnishment entirely.
function weeklyProtectedFloor(st) {
  if (st.consumerDebtPercent === 0) return null; // prohibited
  if (st.slug === 'new-york') return 30 * st.stateMinimumWage;
  if (st.slug === 'illinois') return 45 * st.stateMinimumWage;
  if (st.slug === 'massachusetts') return 50 * Math.max(7.25, st.stateMinimumWage);
  if (st.slug === 'california') return 48 * st.stateMinimumWage; // CCP §706.050 (SB 1477)
  if (st.weeklyFloorOverride != null) return st.weeklyFloorOverride;
  const wage = st.usesStateMinWage ? st.stateMinimumWage : 7.25;
  return st.consumerDebtMinimumWageMultiplier * wage;
}

// Plain-English label for the maximum a creditor can take for ordinary consumer debt.
function consumerCapLabel(st) {
  if (st.consumerDebtPercent === 0) return 'Not allowed';
  if (st.slug === 'new-york') return '10% of gross';
  if (st.slug === 'illinois' || st.slug === 'massachusetts') return '15% of gross';
  if (st.slug === 'california') return '20% of disposable';
  if (st.slug === 'minnesota') return '10–25% of disposable (by income)'; // Minn. Stat. §571.922 graduated tiers
  if (st.slug === 'nevada') return '18–25% of disposable (by income)'; // NRS 31.295: 18% if gross ≤ $770/wk
  return `${st.consumerDebtPercent}% of disposable`;
}
...
function buildComparePage() {
  const title = 'How Much of Your Paycheck Each State Protects From Garnishment (2026)';
  const description = 'A verified, state-by-state comparison of 2026 wage garnishment limits: the maximum a creditor can take, the weekly earnings fully protected, and the governing statute for all 50 states.';
  const canonical = buildCanonical('/compare');
  const verifiedDate = 'September 1, 2026';

  const prohibitionStates = stateList.filter((s) => s.consumerDebtPercent === 0);
  const withFloor = stateList.filter((s) => weeklyProtectedFloor(s) != null);
  const strongest = [...withFloor].sort((a, b) => weeklyProtectedFloor(b) - weeklyProtectedFloor(a))[0];
  const strongestFloor = weeklyProtectedFloor(strongest);

  const rows = stateList.map((st) => {
    const floor = weeklyProtectedFloor(st);
    const floorText = floor == null ? '100% protected' : money(floor) + '/wk';
    const floorSort = floor == null ? 999999 : Math.round(floor); // prohibited sorts as most protective
    const capText = consumerCapLabel(st);
    const capSort = st.consumerDebtPercent === 0 ? 0
      : (st.slug === 'new-york' ? 10 : (st.slug === 'illinois' || st.slug === 'massachusetts' ? 15 : st.consumerDebtPercent));
    const cs = `${st.childSupportPercentCurrent}–${Math.min(st.childSupportPercentSingle + (st.childSupportArrearsExtra || 0), 65)}%`;
    return `
      <tr class="border-b border-slate-100 hover:bg-slate-50">
        <td class="py-3 pl-4 pr-3 font-medium text-slate-900"><a class="text-blue-700 hover:underline" href="/${st.slug}-wage-garnishment-calculator">${escapeHtml(st.name)}</a></td>
        <td class="px-3 py-3 text-slate-700" data-sort="${capSort}">${escapeHtml(capText)}</td>
        <td class="px-3 py-3 font-semibold text-slate-900" data-sort="${floorSort}">${escapeHtml(floorText)}</td>
        <td class="px-3 py-3 text-slate-700">${escapeHtml(cs)}</td>
        <td class="hidden px-3 py-3 text-slate-500 sm:table-cell">${escapeHtml(st.statuteReference || '—')}</td>
      </tr>`;
  }).join('');

  const th = (label, key, extra = '') => `<th scope="col" class="cursor-pointer select-none px-3 py-3 text-left font-semibold text-slate-700 hover:text-teal-700 ${extra}" data-key="${key}">${escapeHtml(label)} <span class="text-slate-300" aria-hidden="true">↕</span></th>`;

  const content = buildPageShell({
    pathname: '/compare',
    eyebrow: 'Flagship 2026 Dataset',
    title: 'How Much of Your Paycheck Each State Protects',
    intro: 'A creditor with a judgment can garnish wages in most states — but how much, and how much of your pay is shielded no matter what, varies enormously. This table shows the 2026 consumer-debt limits for all 50 states, verified against each state’s statute.',
    breadcrumb: [
      { label: 'Home', href: '/' },
      { label: 'Compare', href: '/compare' }
    ],
    content: `
      <div class="rounded-2xl border border-teal-200 bg-teal-50 p-5 text-sm leading-6 text-teal-900">
        <p><strong>The short version:</strong> ${prohibitionStates.length} states (${prohibitionStates.map((s) => escapeHtml(s.name)).join(', ')}) bar wage garnishment for ordinary consumer debt entirely. Everywhere else, a hard floor of weekly earnings is protected before any garnishment applies — from the federal minimum of ${money(217.5)}/week up to ${escapeHtml(strongest.name)}’s ${money(strongestFloor)}/week. Federal law caps most consumer garnishment at 25% of disposable pay; child support can reach 50–65%, and defaulted federal student loans, 15%.</p>
      </div>

      <div class="mt-6 overflow-x-auto rounded-2xl bg-white shadow-sm ring-1 ring-slate-200">
        <table id="gc-compare" class="w-full min-w-[640px] border-collapse text-sm">
          <thead class="border-b border-slate-200 bg-slate-50 text-xs uppercase tracking-wide">
            <tr>
              <th scope="col" class="cursor-pointer select-none py-3 pl-4 pr-3 text-left font-semibold text-slate-700 hover:text-teal-700" data-key="name">State <span class="text-slate-300" aria-hidden="true">↕</span></th>
              ${th('Max creditor can take', 'cap')}
              ${th('Weekly pay protected', 'floor')}
              ${th('Child support', 'cs')}
              <th scope="col" class="hidden px-3 py-3 text-left font-semibold text-slate-700 sm:table-cell">Statute</th>
            </tr>
          </thead>
          <tbody>${rows}</tbody>
        </table>
      </div>
      <p class="mt-3 text-xs text-slate-500">Click any column heading to sort. “Weekly pay protected” is the disposable earnings floor exempt from consumer-debt garnishment; states that prohibit it show as 100% protected. Figures are educational, not legal advice — see each state page and statute for specifics.</p>

      <div class="mt-8 grid gap-6 lg:grid-cols-2">
        <article class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <h2 class="text-lg font-semibold text-slate-900">Cite this data</h2>
          <p class="mt-3 text-sm leading-6 text-slate-700">This dataset is free to cite, quote, and link. Every figure was verified against the state’s governing statute and is kept current with 2026 minimum-wage and exemption changes.</p>
          <p class="mt-3 rounded-lg bg-slate-50 p-3 text-xs leading-5 text-slate-600 ring-1 ring-slate-200">Garnishment Calculator, “How Much of Your Paycheck Each State Protects From Garnishment (2026).” garnishmentcalculator.com/compare. Verified ${escapeHtml(verifiedDate)}.</p>
          <p class="mt-3 text-sm leading-6 text-slate-700">Journalists and researchers: I can provide the computed garnishment figures for any state, pay frequency, and income — free, no strings. <a class="font-semibold text-blue-700 hover:underline" href="/contact">Request the numbers</a>.</p>
        </article>
        <article class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <h2 class="text-lg font-semibold text-slate-900">How to read the table</h2>
          <ul class="mt-3 space-y-2 text-sm leading-6 text-slate-700">
            <li><strong>Max creditor can take</strong> — the ceiling for ordinary consumer debt (credit cards, medical). Child support, taxes, and student loans follow separate, higher limits.</li>
            <li><strong>Weekly pay protected</strong> — earnings below this line can’t be garnished for consumer debt at all, no matter how much is owed.</li>
            <li><strong>Child support</strong> — the CCPA range (50% supporting another family, up to 65% with 12+ weeks of arrears).</li>
          </ul>
          <p class="mt-4 text-sm"><a href="/states" class="font-semibold text-blue-700 hover:underline">Open any state for the full breakdown →</a></p>
        </article>
      </div>
      <p class="mt-6 text-xs text-slate-400">Data verified ${escapeHtml(verifiedDate)} against state statutes and 2026 minimum-wage schedules. Sources include each state’s garnishment statute (linked on the individual state pages) and the federal Consumer Credit Protection Act (15 U.S.C. § 1673).</p>
    `
  });
  ...
  const jsonLd = safeJsonLd({
    '@context': 'https://schema.org',
    '@type': 'Dataset',
    name: 'U.S. State Wage Garnishment Limits (2026)',
    description,
    url: canonical,
    creator: { '@type': 'Organization', name: 'Garnishment Calculator', url: SITE_URL },
    dateModified: '2026-07-18',
    license: `${SITE_URL}/disclosure`,
    isAccessibleForFree: true,
    keywords: ['wage garnishment', 'garnishment limits by state', 'disposable earnings', 'CCPA', 'consumer debt']
  });
  return { title, description, canonical, content, jsonLd };
}
...
function buildGuidePage() {
  ...
  return { title, description, canonical, content, robots: 'noindex,follow', jsonLd: safeJsonLd({ '@context': 'https://schema.org', '@type': 'WebPage', name: title, url: canonical, description }) };
}
...
function generateSitemapXml() {
  ...
  addUrl('/compare');
  addUrl('/resources');
  // /guide is a post-signup thank-you/download page (noindex) — kept out of the sitemap to avoid Soft 404 flags
  addUrl('/disclosure');
...
function buildCalculatorSection(state) {
  ...
  const dataBlob = `<script type="application/json" id="gc-state-data">${JSON.stringify({
    ...
    weeklyFloorOverride: state.weeklyFloorOverride,
    headOfHouseholdRule: state.headOfHouseholdRule
  }).replace(/</g, '\\u003c')}</script>`;
...
${state.headOfHouseholdRule ? `          <div id="gc-hoh-options" style="display:none" class="rounded-xl bg-slate-50 p-4 ring-1 ring-slate-200">
            <label class="flex items-start gap-2 text-sm text-slate-700"><input id="gc-hoh" type="checkbox" class="mt-1" /> <span>I am the head of my family &mdash; I provide more than half the support for a child or other dependent</span></label>
            <p class="mt-2 text-xs leading-5 text-slate-500">${escapeHtml(state.name)} grants a head-of-family exemption for consumer debts. You normally have to claim it in writing after you are served &mdash; it is not applied automatically.</p>
          </div>
