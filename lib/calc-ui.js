...
(function () {
  ...
  var hohOpts = $('gc-hoh-options');
  function syncTypeOptions() {
    if (csOpts) csOpts.style.display = typeSel.value === 'child_support' ? '' : 'none';
    // the head-of-family exemption is a consumer-debt exemption only — it does not
    // limit child support, tax levies, or federal student-loan withholding
    if (hohOpts) hohOpts.style.display = typeSel.value === 'consumer' ? '' : 'none';
  }
  ...
  form.addEventListener('submit', function (e) {
    ...
    var r = GCMath.calculate(st, {
      ...
      headOfHousehold: $('gc-hoh') ? $('gc-hoh').checked : false
    });
